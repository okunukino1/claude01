...
function mysql_cache_table() {
  $table = defined('GEOCODE_CACHE_DB_TABLE') ? (string)GEOCODE_CACHE_DB_TABLE : (defined('GEOCODE_CACHE_TEST_DB_TABLE') ? (string)GEOCODE_CACHE_TEST_DB_TABLE : 'delivery_geocode_cache');
  if (!preg_match('/^[A-Za-z0-9_]+$/', $table)) {
    throw new RuntimeException('DBテーブル名が不正です');
  }
  return $table;
}

function mysql_cache_max_items() {
  $max = defined('GEOCODE_CACHE_DB_MAX_ITEMS') ? (int)GEOCODE_CACHE_DB_MAX_ITEMS : (defined('GEOCODE_CACHE_TEST_DB_MAX_ITEMS') ? (int)GEOCODE_CACHE_TEST_DB_MAX_ITEMS : 50000);
  return max(1000, min($max, 500000));
}

function geocode_cache_admin_pin() {
  if (defined('GEOCODE_CACHE_ADMIN_PIN') && (string)GEOCODE_CACHE_ADMIN_PIN !== '') {
    return (string)GEOCODE_CACHE_ADMIN_PIN;
  }
...
function mysql_cache_pdo() {
  $host = defined('GEOCODE_CACHE_DB_HOST') ? (string)GEOCODE_CACHE_DB_HOST : (defined('GEOCODE_CACHE_TEST_DB_HOST') ? (string)GEOCODE_CACHE_TEST_DB_HOST : '');
  $db = defined('GEOCODE_CACHE_DB_NAME') ? (string)GEOCODE_CACHE_DB_NAME : (defined('GEOCODE_CACHE_TEST_DB_NAME') ? (string)GEOCODE_CACHE_TEST_DB_NAME : '');
  $user = defined('GEOCODE_CACHE_DB_USER') ? (string)GEOCODE_CACHE_DB_USER : (defined('GEOCODE_CACHE_TEST_DB_USER') ? (string)GEOCODE_CACHE_TEST_DB_USER : '');
  $pass = defined('GEOCODE_CACHE_DB_PASSWORD') ? (string)GEOCODE_CACHE_DB_PASSWORD : (defined('GEOCODE_CACHE_TEST_DB_PASSWORD') ? (string)GEOCODE_CACHE_TEST_DB_PASSWORD : '');
  foreach (['host' => $host, 'name' => $db, 'user' => $user, 'password' => $pass] as $name => $value) {
    if ($value === '') {
  ...
  $port = defined('GEOCODE_CACHE_DB_PORT') ? (int)GEOCODE_CACHE_DB_PORT : (defined('GEOCODE_CACHE_TEST_DB_PORT') ? (int)GEOCODE_CACHE_TEST_DB_PORT : 3306);
  $charset = 'utf8mb4';
