...
function call_segment_directions($profile, $start, $destination, $token, $heading, $useCurb) {
...
$attempts = [
  ['profile' => 'mapbox/driving-traffic', 'heading' => $heading, 'curb' => true],
  ['profile' => 'mapbox/driving-traffic', 'heading' => null, 'curb' => true],
  ['profile' => 'mapbox/driving', 'heading' => null, 'curb' => true],
  ['profile' => 'mapbox/driving', 'heading' => null, 'curb' => false],
];

$lastAttempt = null;
$attemptedSettings = [];
foreach ($attempts as $settings) {
  $settingsKey = $settings['profile'] . '|' . ($settings['heading'] === null ? '' : (string)$settings['heading']) . '|' . ($settings['curb'] ? '1' : '0');
