...
- テスト版は **v2026.06.24-test.91**。インストール型Androidアプリの戻る三角をアプリ内で受け取り、写真拡大中は編集画面へ、`配送先を編集` の表示中はマップへ戻る。
- Androidシェルへ公式 `@capacitor/app` **8.1.1** を追加。加えて、同プラグインが入っていない既存APK向けにWeb履歴を使う互換処理を追加したため、この戻る操作の改善だけならAPKの再インストールは不要。
