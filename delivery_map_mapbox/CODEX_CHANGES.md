...
- テスト版は **v2026.06.24-test.90**。インストール型Androidアプリの戻る三角をアプリ内で受け取り、写真拡大中は編集画面へ、`配送先を編集` の表示中はマップへ戻る。地図上で押した場合だけ従来どおりアプリを終了する。
- Androidシェルへ公式 `@capacitor/app` **8.1.1** を追加。Web側だけでなくAPKの更新インストールが必要。
