...
- 作業開始基準は最新 `origin/main` のコミット `fb10b84b7bcf2e2ad6a561de838cda28db8d55f3`。
- 安定版は引き続き **v2026.07.10-1**。今回の変更は反映していない。
- テスト版は **v2026.06.24-test.98**。
- インストール型Androidアプリのネイティブカメラで、EXIFの向きをJPEG本体へ反映し、アプリの縦向きに合わせて撮影方向を固定する。OCRの回転再試行で読み取れた写真は、その追加回転も保存写真へ引き継ぐ。ブラウザー・ギャラリー画像もEXIFを反映してからJPEGへ正規化する。
