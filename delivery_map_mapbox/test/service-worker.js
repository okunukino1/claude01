...
const SHELL_CACHE = 'rys-test-shell-v1';
const STATIC_CACHE = 'rys-test-static-v1';
...
function isAppShellPath(url) {
  return url.pathname.endsWith('/delivery_map_mapbox/test/') ||
         url.pathname.endsWith('/delivery_map_mapbox/test/index.html');
}
...
function isStaticAsset(url) {
  ...
  // アプリアイコン (?v= で版管理されている)
  if (url.pathname.includes('/delivery_map_mapbox/test/app-icon-')) return true;
  return false;
}
...
self.addEventListener('push', event => {
  ...
  const options = {
    body: data.body || '',
    icon: new URL('./app-icon-192.png?v=20260616-test-17', self.registration.scope).href,
    badge: new URL('./app-icon-192.png?v=20260616-test-17', self.registration.scope).href,
